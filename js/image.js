			...
			if(data.data != '') {
				data.data.forEach(function(image) {
					if(image.id == image_id) {                        
						image_container.innerHTML = '<img class="w-100 my-2 border" src="img/' + image.name + '" />';
						user_list.forEach(function(user){
							if(image.user_id == user.id) {
								if(user.image == null) {
									author_image.innerHTML = '<a href="user.html?id=' + user.id + '"><img class="w-100 shadow border border-info d-block author-image rounded" src="img/no-img.png"></a>';
								} else {
									author_image.innerHTML = '<a href="user.html?id=' + user.id + '"><img class="w-100 shadow border border-info d-block author-image rounded" src="img/' + user.image + '"></a>';
								}
								author_name.innerHTML = '<h1 class="author-name text-center mt-3 mb-3">' + user.name + '</h1>';
							}
						});
					}
				});
	...
	comment_list.forEach(function(comment){
		user_list.forEach(function(user){
			if(comment.user_id == user.id) {
				// comments_container.innerHTML += '<p class="coment-author d-inline">';// Obrisan je i </p> comments_container.innerHTML += `<p class="comment-author-name d-inline">${user.name} </p>  -->>> </p>`;......
				if(user.image == null) {                    
					comments_container.innerHTML += '<a href="users.html?id=' + user.id + '"><img class="comment-author-image w-25" src="img/no-img.png" /></a>';
				} else {
					comments_container.innerHTML += '<a href="users.html?id=' + user.id + '"><img class="comment-author-image w-25" src="img/' + user.image + '" /></a>';
				}
				comments_container.innerHTML += `<p class="comment-author-name">${user.name} </p>`;
			}
		});
